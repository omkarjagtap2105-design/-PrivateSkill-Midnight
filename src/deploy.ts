/**
 * Deploy the PrivateSkill contract to a Midnight network (undeployed by default;
 ...
 *
 * The contract's constructor takes one argument — the owner id
 * (a domain-separated hash of the owner's secret key). Nothing sensitive is
 * passed in or printed; only public ids and the contract address are recorded.
 */
...
import {
  ...
  waitForProofServer,
  PRIVATE_STATE_ID,
} from './private-skill/harness';
import { deriveActorKeys } from './private-skill/actors';
import { ownerIdFor, toHex } from './private-skill/crypto';
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║  Deploy PrivateSkill to ' + network);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  // Constructor argument: the public owner id, a domain-separated hash of the
  // owner's secret key. The secret key stays in memory — only the public id
  // is placed on the ledger.
  const { ownerKey } = deriveActorKeys(SEED);
  const ownerId = ownerIdFor(ownerKey);
  console.log(`  Owner id (public): 0x${toHex(ownerId)}\n`);
